...
AVATTO2_SENSOR_ATTR = 0x046E  # [1], [2], [3]
AVATTO2_TEMP_CALIBRATION_ATTR = 0x0213  # temperature calibration (degree)
AVATTO2_MIN_TEMPERATURE_VAL = 500  # minimum limit of temperature setting (degree/100)
AVATTO2_MAX_TEMPERATURE_VAL = 9500  # maximum limit of temperature setting (degree/100)
...
class Avatto2ManufCluster(TuyaManufClusterAttributes):
    ...
    attributes.update(
        {
            ...
            AVATTO2_SENSOR_ATTR: ("sensor_choose", t.uint8_t, True),
            AVATTO2_TEMP_CALIBRATION_ATTR: ("temperature_calibration", t.int32s, True),
        }
    )

    DIRECT_MAPPED_ATTRS = {
        AVATTO2_TEMPERATURE_ATTR: (
            "local_temperature",
            lambda value: value * 10,
        ),
        AVATTO2_TARGET_TEMP_ATTR: (
            "occupied_heating_setpoint",
            lambda value: value * 10,
        ),
        AVATTO2_TEMP_CALIBRATION_ATTR: (
    ...
    def _update_attribute(self, attrid, value):
        ...
        elif attrid == AVATTO2_MODE_ATTR:
            self.endpoint.device.thermostat_bus.listener_event("mode_change", value)
        elif attrid == AVATTO2_TEMP_CALIBRATION_ATTR:
...
class Avatto2Thermostat(TuyaThermostatCluster):
    ...
    attributes.update(
        {
            0x4000: ("sensor_choose", t.uint8_t, True),
    ...
    DIRECT_MAPPING_ATTRS = {
        ...
        "sensor_choose": (AVATTO2_SENSOR_ATTR, None),
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.endpoint.device.thermostat_bus.listener_event(
            ...
            AVATTO2_MIN_TEMPERATURE_VAL,
        )
        self.endpoint.device.thermostat_bus.listener_event(
            "temperature_change",
            "max_heat_setpoint_limit",
            AVATTO2_MAX_TEMPERATURE_VAL,
        )
    ...
    def map_attribute(self, attribute, value):
        ...
        if attribute == "operation_preset":
            if value == 1:
                return {AVATTO2_MODE_ATTR: 1}
            if value == 2:
                return {AVATTO2_MODE_ATTR: 0}
...
class Avatto2UserInterface(TuyaUserInterfaceCluster):
    ...
    _CHILD_LOCK_ATTR = AVATTO2_CHILD_LOCK_ATTR


class Avatto2ChildLock(CustomTuyaOnOff):
    ...
    def map_attribute(self, attribute, value):
        """Map standardized attribute value to dict of manufacturer values."""
        if attribute == "on_off":
            return {AVATTO2_CHILD_LOCK_ATTR: value}


class Avatto2TempCalibration(LocalDataCluster, AnalogOutput):
    ...
    async def write_attributes(self, attributes, manufacturer=None):
        ...
        for attrid, value in attributes.items():
            ...
            await Avatto2ManufClusterSelf[
                ...
                {AVATTO2_TEMP_CALIBRATION_ATTR: value * 10},
                manufacturer=None,
            )
        return ([foundation.WriteAttributesStatusRecord(foundation.Status.SUCCESS)],)


class Avatto2(EnchantedDevice, TuyaThermostat):
    """Avatto2 Thermostatic radiator valve."""
    ...
    replacement = {
        ENDPOINTS: {
            ...
            2: {
                ...
                INPUT_CLUSTERS: [Avatto2ChildLock],
                OUTPUT_CLUSTERS: [],
            },
            3: {
                PROFILE_ID: zha.PROFILE_ID,
                DEVICE_TYPE: zha.DeviceType.CONSUMPTION_AWARENESS_DEVICE,
                INPUT_CLUSTERS: [Avatto2TempCalibration],
                OUTPUT_CLUSTERS: [],
            },
            242: {
                PROFILE_ID: 41440,
